...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir=".", names=(), eps=1e-16, prefix=""): # P and R are calculated here
    """
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False,Ioa=False, eps=1e-7):
    """
    ...
    if xywh:  # transform from xywh to xyxy, first form of bounding box definition
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
        w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
        b1_x1, b1_x2, b1_y1, b1_y2 = x1 - w1_, x1 + w1_, y1 - h1_, y1 + h1_
        b2_x1, b2_x2, b2_y1, b2_y2 = x2 - w2_, x2 + w2_, y2 - h2_, y2 + h2_
    else:  # x1, y1, x2, y2 = box1, second  form of bounding box definition
    ...
    #if Ioa:
        # Area of box2 (the predicted box)
     #   box2_area = w2 * h2 + eps

        # IoA (Intersection over Area of box2)
      #  ioa = inter / (w2 * h2 + eps)

        # Calculate how much of the predicted box is contained within all the N boxes in the training data
       # containment = ioa.max(dim=-1)[0]  # Find the max containment for each predicted box
        #return 1 - containment  # The loss is 1 minus the containment
    box2_area = w2 * h2 + eps
    box1_area = w1 * h1 + eps
    # Union Area
    union = w1 * h1 + w2 * h2 - inter + eps
    ioa = 0.5 * (inter / box2_area) + ((inter / box1_area))

    # IoU, also building here CIou, GIou and DIou
    iou = inter / union #maybe switch union to box2_area
...
def bbox_ioa(box1, box2, eps=1e-7): #Can be good! Intersection over area, almost what Ronit talked about
    """
